...
class ContinuousHealthMonitor:
    ...
    def monitor_camera(self,
        ...
        Args:
            camera_index: Integer index of the camera device
            interval_seconds: Time between analyses
            max_iterations: Stop after this many captures (None=infinite)
            save_frames: Whether to save each captured frame to disk
            frame_save_dir: Directory to save frames into if --save-frames is used
            display: If True, show a real-time video window with prediction overlay
        """
...
def main():
    ...
    parser.add_argument(
        "--display",
        action="store_true",
        help="Show camera feed with prediction overlay (press 'q' to quit)"
    )
